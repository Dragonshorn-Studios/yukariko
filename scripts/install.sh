...
if command -v systemctl >/dev/null 2>&1 && [ -d /run/systemd/system ]; then
  if curl -fsSL -o "${TMP}/yukariko.service" "${DL}/${VERSION}/yukariko.service"; then
    ...
    # directives append, so the data-dir ReadWritePaths entry stays. The
    # file is MERGED, never truncated: `yukariko expose` grants paths in
    # this same file and those grants must survive installer reruns.
    if [ -n "${RW_PATHS}" ] || [ "$HOME_READ" -eq 1 ]; then
      DROPIN="${SYS_UNIT}.d/10-writables.conf"
      mkdir -p "${SYS_UNIT}.d"
      WANT="${TMP}/writables-want"
      {
      ...
      if [ ! -f "${DROPIN}" ]; then
        {
          echo "# Managed jointly by scripts/install.sh and yukariko expose;"
          echo "# merged, never truncated."
          echo "[Service]"
          cat "${WANT}"
        } > "${DROPIN}"
      else
        while IFS= read -r LINE; do
          [ -n "$LINE" ] || continue
          grep -Fxq -- "$LINE" "${DROPIN}" || printf '%s\n' "$LINE" >> "${DROPIN}"
        done < "${WANT}"
      fi
      note "updated ${DROPIN} (merged)"
    fi
