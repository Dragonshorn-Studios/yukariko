...
| `internal/ui/templates/*.html` | Hand-written Go `html/template` files. No third-party layouts or component libraries. The visible chrome is Status / Versions / Health / Logs with a Chronicle side panel; template filenames remain sanctuary / vestments / divination / chronicle. Live updates use a meta refresh (no JavaScript). |
