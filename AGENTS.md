...
- OIDC gate (`#61`): `internal/auth` puts `/ui` and `/api/v1` behind an optional OpenID Connect login (any compliant IdP; Authentik documented) — fail-closed config (`auth.oidc` requires `server.enabled`, an https-or-loopback issuer and `redirect_base`, a client id, a `SecretRef` client secret, openid in scopes). The flow is authorization-code + PKCE (S256) with go-oidc verifying ID tokens (JWKS signature, iss/aud/exp/nonce; RS256/ES256 allowlist); `state`/`nonce`/verifier live in single-use `auth_pending` rows (10-minute TTL), and sessions are `web_sessions` rows keyed by the SHA-256 of a 256-bit cookie token (HttpOnly, SameSite=Lax, `Secure` + `__Host-` on https origins; migration 3; absolute TTL swept at startup and login). The provider is discovered lazily — an unreachable IdP never blocks daemon startup. `Protect` gates only `/ui` and `/api`: `/auth/*` serves the flow and `/report/v1/events` keeps its HMAC channel. Unauthenticated: API gets JSON 401s, dashboard navigations get a login 302, fetch-style requests get 401 (live.js then navigates). Optional `allowed_groups` is a default-deny membership check against the verified `groups` claim — defense in depth over the IdP's own app policy. The client secret resolves only at token exchange and never reaches logs or storage; reverse-proxy headers are never trusted. Auth off must leave the HTTP surface byte-identical to before.
