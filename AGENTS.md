...
Work GitHub issues **in issue-map order** (#1 → #19 for the MVP; later issues as filed).
...
MVP complete (#1–#19, scope frozen); #42 is the first post-MVP extension.
...
- `internal/auth` — optional OIDC gate (#61): session middleware, /auth handlers, SQLite sessions
- `internal/health` — HTTP/docker/command probes, post-deploy checks, independent monitor
- `internal/deploy` — compose pipeline (#11); standalone recreation (#12); operator restart (#56); Docker CLI engine
...
- Stdlib first. Cobra is the CLI router only; do not add Viper. YAML parsing is `gopkg.in/yaml.v3` with `KnownFields(true)` strict decoding (`internal/config`); keep it the only YAML dependency. SQLite runs on `modernc.org/sqlite` (pure Go, no cgo) behind `database/sql` (`internal/store`): WAL, busy timeout, forward-only migrations recorded in `schema_migrations` — never edit an applied migration, append a new one. The deliberate auth exception (#61): OIDC relying-party verification goes through `github.com/coreos/go-oidc/v3` + `golang.org/x/oauth2` (pure Go, no cgo) — hand-rolled auth verification is a non-goal, the same way `internal/registry` hand-rolls only transport, never crypto it could get wrong silently.
- Docker access is CLI-only (`#5`): `internal/docker` builds `docker ps` / `docker inspect` argv and executes them through `internal/runner`; no Docker SDK or socket dependency, now or later. Docker endpoints (rootless/multi-daemon hosts, `#42`) are argv too — `docker.CLIClient.GlobalFlags` carries the resolved `--context`/`-H` selector into every docker/compose invocation; user-owned argv (compose steps, health commands) gets `DOCKER_HOST`/`DOCKER_CONTEXT` env instead. Discovery records environment-variable **names only** — inspect values must never enter results, logs, or storage — and the runner used for discovery must not attach a persistence `Sink`. Docker daemon access is host-equivalent privilege; see `docs/docker-access.md`.
...
- Daemon wiring (`#14`): `internal/daemon.Assemble` builds every component and the store adapters; `SourceChecker` routes git/registry checks and records observations (kind `git_sha`, or `digest:<canonical ref>` per image) separately from deployed versions; `DeployDispatcher` opens a deployment row per run and binds its checkpoint to `CommitDeploymentSuccess` — the only path that advances the deployed version. Commands `run/check/update/status/logs` each have a themed alias implemented as the same cobra command (behaviorally identical by construction); `check` never deploys, `update --dry-run` runs check+preflight only, and all commands take `--config` plus a data dir: a root invocation on Linux defaults to the installer layout (`/etc/yukariko/yukariko.yaml`, `/var/lib/yukariko` — provisioned by `scripts/install.sh`; a missing default config errors with an installer pointer), otherwise `--config` is required and the data dir defaults to `./data` (`internal/cli/defaults.go`); a root CLI pass also hands root-created store files to the data dir's owner so the systemd service user is never locked out of its store. `expose` (no alias, `#44`) is the operator-side counterpart to the strict unit: as root it appends one resolved git-worktree or rootless-socket path to the unit's `ReadWritePaths` drop-in idempotently (auto `ProtectHome=read-only` under `/home`, `/root`, and `/run/user`) and always reloads systemd; it also applies the matching filesystem ACLs for the unit's user (`rwX`+defaults on worktrees, `rw` on sockets, traverse on the owning `/home/<user>`//`/run/user/<uid>` tree — skipped for root units, reapplied every run because rootless sockets are recreated), the installer provisions the `acl` package (#51), and the installer suggests expose when it finds rootless sockets under `/run/user/*/docker.sock` but never grants anything itself.
- Operator restart (`#56`): `restart --app <id>` (required app id; no `--all`) bounces Compose via `docker compose … restart` with the exact configured context (`-f`, `--env-file`, `--profile`, `-p`, workdir) or standalone via `docker restart <name>`, honoring per-app `docker:` endpoint flags. It takes the same per-app lock as `update` (scheduler queue + store unique-index; the running row is a lock token deleted afterwards so restart is not a deployment record) so it cannot overlap a deploy, records success/failure events (`kind=restart`), and never advances the deployed-version checkpoint. No HTTP/UI mutation, no health-triggered restart, no daemon restart.
- Read-only API (`#15`): `internal/httpapi` serves GET/HEAD-only JSON (`/api/v1/apps`, `/apps/{id}`, `/hosts`, `/deployments`, `/logs`, `/health`) with security headers, capped pagination, and JSON errors; route projections live in `internal/state` (shared with the CLI) to keep `httpapi` free of daemon imports. Remote-host availability derives deterministically from heartbeat age (online ≤ stale-after ≤ stale ≤ offline-after; default 3m/10m) and absence is never healthy. Local/remote records expose source and age. The bind defaults to loopback; exposure is an operator decision behind existing access controls or the opt-in OIDC gate (#61). No route can mutate anything.
- OIDC gate (`#61`): `internal/auth` puts `/ui` and `/api/v1` behind an optional OpenID Connect login (any compliant IdP; Authentik documented) — fail-closed config (`auth.oidc` requires `server.enabled`, an https-or-loopback issuer and `redirect_base`, a client id, a `SecretRef` client secret, openid in scopes). The flow is authorization-code + PKCE (S256) with go-oidc verifying ID tokens (JWKS signature, iss/aud/exp, RS256/ES256 allowlist; nonce is compared by Yukariko against the single-use pending row); `state`/`nonce`/verifier live in single-use `auth_pending` rows (10-minute TTL), and sessions are `web_sessions` rows keyed by the SHA-256 of a 256-bit cookie token (HttpOnly, SameSite=Lax, `Secure` + `__Host-` on https origins; migration 3; absolute TTL swept at startup and login). The provider is discovered lazily with a short failure backoff — an unreachable IdP never blocks daemon startup or serializes logins. `Protect` gates only `/ui` and `/api`: `/auth/*` serves the flow and `/report/v1/events` keeps its HMAC channel; a failing session store answers 503, never a login redirect. Unauthenticated: API gets JSON 401s, dashboard navigations get a login 302, fetch-style requests get 401 (live.js then navigates — on 401 only, transient failures keep the quiet retry). Optional `allowed_groups` is a default-deny membership check against the verified `groups` claim — defense in depth over the IdP's own app policy. The client secret resolves only at token exchange and never reaches logs or storage; reverse-proxy headers are never trusted; failure paths log via `log/slog` while client messages stay generic. Auth off must leave the HTTP surface byte-identical to before.
...
- Dashboard (`#16`, visual lock `#58`): `internal/ui` server-renders with html/template + embed — no forms, no build chain, no frameworks. A tiny original `live.js` is progressive enhancement: JS browsers GET the same HTML and swap `#main` + footer (meta refresh is removed so the page does not full-reload); without JS the meta refresh remains (12s idle, 5s while any deployment is running). An in-progress app renders an Updating chip with a CSS spinner (static under prefers-reduced-motion). The four sections (Status overview, Versions, Health, Logs) render from the same `internal/state` projections as the CLI/API; template filenames stay sanctuary/vestments/divination/chronicle. Status is a sparse app-card list plus a Chronicle side panel, not an identity table. Running/health/update/deployment stay separate facts with separate badge classes (they cannot be confused visually or textually); remote source/age and stale/offline are prominent. All assets are original (docs/ASSETS.md inventories them). The locked palette is canvas/surface/ink/lapis/mirage/rose-gold/ok/fail with a tiny faceted diamond mark, reduced-motion, and responsive rules — no chapel navy/gold.
...
- #42 Docker endpoint selection: rootless and multiple local daemons
- #44 Rootless socket discovery hints and a yukariko expose command
- #56 CLI: restart a configured app without deploying
- #61 Optional OIDC (Authentik) authentication for the dashboard and HTTP API
